...
describe('SubmitDialog', () => {
  ...
  it('renders nothing when isOpen is false', () => {
    const { container } = render(<SubmitDialog isOpen={false} onClose={mockOnClose} onConfirm={mockOnConfirm} />);
    expect(container.innerHTML).toBe('');
  });

  it('renders dialog content when isOpen is true', () => {
    render(<SubmitDialog isOpen={true} onClose={mockOnClose} onConfirm={mockOnConfirm} />);
  ...
  it('calls onConfirm and onClose when Submit is clicked', async () => {
    const user = userEvent.setup();
    render(<SubmitDialog isOpen={true} onClose={mockOnClose} onConfirm={mockOnConfirm} />);
